<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="论坛分类" prop="luntanfenlei">
          <el-input v-model="ruleForm.luntanfenlei" 
              placeholder="论坛分类" clearable  :readonly="ro.luntanfenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	luntanfenlei : false,
      ...
      rules: {
          luntanfenlei: [
                { required: true, message: '论坛分类不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `luntanleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.luntanleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
